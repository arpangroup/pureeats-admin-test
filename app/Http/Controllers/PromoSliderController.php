...
class PromoSliderController extends Controller
{
     ...
     */
    public function promoSliderOld(Request $request)
    {
    ...
    public function promoSlider(Request $request)
    {
        ...
        if (Cache::has('main-slider')) {
            $mainSliders = Cache::get('main-slider');
        } else {
            $mainSliders = PromoSlider::where('is_active', 1)
                ->whereIn('slider_type', ['BANNER', 'TOP_PICK', 'CUSINE', 'DISH_CATEGORY'])
                ->with('slides')->get();
        ...
        return response()->json([
            'success' => true,
            'message' => 'Total ' .sizeof($mainSliders) .' slider found',
            'data' => $mainSliders
        ]);
    }
}
